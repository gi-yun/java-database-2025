...
- JOIN[SQL](./Day03/sql03_join기본.sql)
    ...
    - 카티션곱
        ...
        - 조인 이전에 데이터 확인 - 실무에서 카티션 곱으로 데이터를 사용할 일이 절대! 없음.
    - 내부조인
...
- DML : [INSERT쿼리](./Day04/sql01_INSERT.sql), [UPDATE/DELETE쿼리](./Day04/sql02_UPDATE_DELETE.sql)
    ...
    - DELETE - 데이터 삭제.
        ...
        DELETE FROM 테이블 명
            [WHERE조건]
        ```
- 트랙젝션 : [트랜잭션쿼리](./Day04/sql03_트랜잭션.sql)
    - 논리적인 처리단위.
    - 은행에서 돈을 찾을 때 아주 많은 테이블 접근해서 일을 처리
        - 적어도 일곱여덟개 이상의 테이블 접근해서 조회하고 업데이트 수정.
        - 제대로 일이 처리안되면 다시 원상복귀
        <!-- - atm 기계정보 테이블, 회원테이블 비밀번호 확인, 계좌정보, 계좌금액 테이블,
        - 계좌정보테이블, -계좌금액입출금내역테이블금액빼기, ATM기계총지폐정보테이블,
        - 영수증관련정보 테이블, atm기계정보테이블, 은행로그테이블 -->
        - DB 설정 AUTO COMMIT 해체 권함
        ```sql
        BEGIN TRANSACTION; -- 트랙잭션 시작(정석 * 오라클은 자동으로 시작)
        commit; -- 트랙잭션 시작
        ROLLBACK; -- 원상복귀
        ```
...
- INDEX : [INDEX쿼리](./Day04/sql05_인덱스.sql), [인덱스용테이블생성](./ref/bulk_data_insert.sql)
    ...
    - 인덱스 종류
        - 클러스터드(Clustered) 인덱스
            ...
            - PK에 없으면 처음으로 설정되는 UNIQUE 제약조건의 컬럼에 생성
...
- VIEW: [뷰쿼리](./Day05/sql01_뷰.sql)
    ...
    - 뷰이라도 INSERT, UPDATE, DELETE가 가능함. 단일 뷰에서만.
...
- 서브쿼리 : [서브쿼리](./Day05/sql02_서브쿼리.sql)
    ...
    - 단일행 서브쿼리, 다중행 서브쿼리 마다 사용법 다름
    - SELECT 서브쿼리, FROM절 서브쿼리, WHERE절 서브쿼리
    - 서브쿼리는 JOIN으로 거의 다 변경가능

- 시퀸스[시퀀스](./Day05/sql03_시퀀스.sql)
...
- 사용자 계정 권한 : [쿼리](./Day05/sql04_사용자계정관리.sql)
    - 사용자 생성 후 권한(ROLE)을 부여해야 스키마를 사용가능
    ...
    - Stored Procedure와 FUNCTION를 만들기위해서 사용
        - 저장프로시저
            ...
            - 실행시 EXEC 사용
                ...
                CALL 프로시저명(파라미터);
                또는
                EXEC 프로시저명 (파라미터); -- 현재 DBEAVER에서 사용불가
                ```
        - 함수
